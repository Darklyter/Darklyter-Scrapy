...
class BasePerformerScraper(BaseScraper):
    ...
    def parse_performer(self, response):
        ...
        if 'birthplace_code' in response.meta and response.meta['birthplace_code']:
            item['birthplace_code'] = response.meta['birthplace_code']
        else:
            item['birthplace_code'] = self.get_birthplace_code(response)
        ...
        # Whichever of the pair a spider supplies, both slots get filled.  When a
        # spider gives both they're kept as-is; when it gives neither, both fall
        # back to the domain.
        if not item['network'] and not item['site']:
            item['network'] = self.get_network(response)
            item['site'] = self.get_site(response)
        elif not item['site']:
            item['site'] = item['network']
        elif not item['network']:
    ...
    def get_name(self, response):
        if 'name' in self.selector_map:
            ...
            return unidecode(string.capwords(self.cleanup_text(name)))
        return ''
    ...
    def get_birthday(self, response):
        if 'birthday' in self.selector_map:
            ...
            if birthday:
                birthday = birthday.replace("-00", "-01")
                parsed = self.parse_date(birthday)
                # No birthday is a valid answer, so an unparseable one is reported
                # rather than raised.
                if parsed is None:
                    self.logger.warning(f"Could not parse birthday '{birthday}' on {response.url}")
                    return ''
