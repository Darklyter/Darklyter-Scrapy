...
class BaseMovieScraper(BaseScraper):
    ...
    def parse_movie(self, response):
        ...
        # The pipeline reads these whether or not a spider bothers to set them, so
        # give them harmless defaults rather than letting it raise a KeyError.
        if 'store' in response.meta:
            item['store'] = response.meta['store']
        elif 'store' not in item:
            item['store'] = ''

        if 'directors' in response.meta:
            item['directors'] = response.meta['directors']
        elif 'directors' not in item:
            item['directors'] = item['director'] if item.get('director') else []
    ...
    def get_date(self, response):
        if 'date' in self.get_selector_map():
            ...
            if scenedate:
                ...
                parsed = self.parse_date(scenedate, date_formats=date_formats)
                # An unparseable date falls back to today, the same as no date at all.
                if parsed is None:
